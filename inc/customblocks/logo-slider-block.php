...
<section id="logo-slider-<?php echo $block_id; ?>" class="dsn-logo-slider-container">
    ...
    <?php if($title) { ?><h2 class="dsn:text-center dsn:mb-4 dsn:text-2xl dsn:md:text-5xl"> 
        <?php echo $title; ?></h2> <?php } ?>
    <?php if($description) { ?><p class="dsn:text-center dsn:lg:max-w-8/12 dsn:mx-auto"><?php  echo $description; ?></p> <?php } ?>
    <div class="dsn:flex dsn:flex-col dsn:lg:flex-row dsn:items-center dsn:gap-20 dsn:mb-10 dsn:md:my-20 dsn:px-0 dsn:text-center dsn:md:text-left dsn:lg:px-0">
        <div class="dsn:w-full dsn:lg:w-5/12">
          <?php if($contentTitle) {?>  <h3><?php echo $contentTitle; ?></h3> <?php } ?>
            <?php if($contentDescription) { ?><p class="dsn:mb-12"><?php echo $contentDescription; ?></p><?php } ?>
            <?php if($contentCTA): ?>
