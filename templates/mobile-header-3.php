<header class="mobile block dsn:lg:hidden dsn:text-[#0988c2] dsn:border-b">
		...
		<div class="mobile-nav-right dsn:relative dsn:bg-white dsn:w-full dsn:h-full dsn:float-right">
			...
			<div class="cart-search-combo cf dsn:flex dsn:justify-center dsn:items-center dsn:px-4 dsn:py-4 dsn:gap-3 dsn:border-1">
				...
				<?php wp_nav_menu(array(
					...
					'menu_class' => 'dsn:block dsn:w-full dsn:text-white dsn:w-full dsn:pl-0',
					'container'			=> "nav",
	...
	<style>
        ...
        .dsn-mobile-hamburger.is-active > .nav-close span::after {
            transform: rotate(-45deg);
            transition: transform 0.5s;
        }
        .dsn-mobile-hamburger.is-active > .nav-close span::before {
            transform: rotate(45deg);
            transition: transform 0.5s;
        }
        .is-active > a span {
		...
		#dsnHTML.dsnOpen, .the-navigation.dsnMobileOpen {
			position: absolute;
			transition: right 0.5s linear;
		}
		.the-navigation.dsnMobileOpen {
		display: block;
		right: -285px;
		}
	}
